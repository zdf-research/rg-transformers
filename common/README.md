...
| `eval/`            | **Modular evaluation scripts.** A suite of `pytest` scripts (`week01_tokenizer.py`, etc.) used by GitHub Actions to automatically score member implementations.      |
...
The evaluation scripts in `eval/` are effectively immutable once the leaderboard starts. If you find a genuine bug in them, open an issue, tag everyone, and wait for group discussion before touching them.
